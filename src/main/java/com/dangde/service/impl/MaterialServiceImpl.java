...
public class MaterialServiceImpl implements MaterialService {
  ...
  @Autowired CommentDao comment_dao;
  @Autowired NoticeDao noticeDao;

  @Autowired OperationDao operationDao;
  @Autowired UserDao userDao;
  @Autowired ApproveDao approveDao;
  ...
  public List<Object> getMaterials(
    ...
    materials =
        this.materialDao.getMaterials(
            operation_id, material_type, queryInfo.getStartindex(), queryInfo.getPagesize());
        ...
        for (int i = 0; i < files.length; i++) {
          ...
          if (!file.isEmpty()) {
            try {
              ...
              else if (file_type == 1) url = "upload/documents/" + l + ext;
              urls += url + " ";
          ...
          if (file_type == 1) { // 文档
            if (material.getFlag_output() == 1) { // 成果界面上传材料
              ...
              Long id =
                  materialDao.material_Cancel_Output_getid(
                      material.getOperation_id(), material.getType());
  ...
  public boolean insert_Comment(Comment comment) {
    ...
    try {
      comment_dao.insertComment(comment);
      return true;
    } catch (Exception e) {
